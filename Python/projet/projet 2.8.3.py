...
def teste_col(y,x):
	global pxg,pyg,points,nbg,pt,combo
	if (x>pxg and x<(pxg+25) and y>pyg and y<(pyg+25))  or ((x+48)>pxg and (x+48)<(pxg+25) and y>pyg and y<(pyg+25))  or (x>pxg and x<(pxg+25) and (y+48)>pyg and (y+48)<(pyg+25))  or  ((x+48)>pxg and (x+48)<(pxg+25) and (y+48)>pyg and (y+48)<(pyg+25)):     #pour le 4 coins du birds
		can.delete(mac)
		stop3()
		...
		if combo > 1:
			print "COMBO x" ,combo
			print "Touche!  + " ,combo*pt,
			points = points + combo*pt
		else:
			points = points + pt		
			print "Touche!  + " ,pt,
		print ":" ,points
...
def teste_col2(y,x):
	global pxg2,pyg2,points,nbg,pt,combo
	if (x>pxg2 and x<(pxg2+25) and y>pyg2 and y<(pyg2+25))  or ((x+48)>pxg2 and (x+48)<(pxg2+25) and y>pyg2 and y<(pyg2+25))  or (x>pxg2 and x<(pxg2+25) and (y+48)>pyg2 and (y+48)<(pyg2+25))  or  ((x+48)>pxg2 and (x+48)<(pxg2+25) and (y+48)>pyg2 and (y+48)<(pyg2+25)):     #pour le 4 coins du birds
		can.delete(mac2)
		stop3_2()
...
def deplace_gland():
	global pxg0,pyg0,tpg,vg0,anggd,anggr,vxg0,vyg0,pxg,pyg,nbg,lvl,points,pt,mac,image_miss,miss
	
	
	if pxg < 1200 and pyg < 365:		
		tpg = tpg + 0.05
		pxg = vxg0*tpg + pxg0
		pyg = (m*g)/2*tpg*tpg - vyg0*tpg + pyg0						
		
	else:
		image_miss = ImageTk.PhotoImage(file='img/rate.png')
		miss = can.create_image(pxg-35, 374, image=image_miss, anchor=NW)
		can.delete(mac)
		stop3()
...
def deplace_gland2():
	global pxg0,pyg0,tpg2,vg02,anggd2,anggr2,vxg02,vyg02,pxg2,pyg2,nbg,lvl,points,pt,mac2,p2,image_miss2,miss2
	
	
	if pxg2 < 1200 and pyg2 < 365:		
		tpg2 = tpg2 + 0.05
		pxg2 = vxg02*tpg2 + pxg0 
		pyg2 = (m*g)/2*tpg2*tpg2 - vyg02*tpg2 + (pyg0-p2)						
		
	else:
		image_miss2 = ImageTk.PhotoImage(file='img/rate.png')
		miss2 = can.create_image(pxg2-35, 379, image=image_miss2, anchor=NW)
		can.delete(mac2)
		stop3_2()
...
def moov():	
	global px,py,px0,py0	
	px = vx0*tps + px0
	py = (m*g)/2*tps*tps - vy0*tps + py0	
													
			
def deplace():
	global px,py,vx0,vy0,px0,py0,tps,flag1,angd,angr,v0,v1,e,lx,ly,lvl,rebon,ab
...
def pos_bird():
	global image_ab,image_ab2,ab,py0,px0,lx,ly,poss
	...
	if poss == "droite":			
		image_ab = ImageTk.PhotoImage(file='img/squirrel.png')							#les birds
		ab = can.create_image(px0, py0, image=image_ab, anchor=NW)
	if poss == "gauche":
		image_ab2 = ImageTk.PhotoImage(file='img/mac.png')							#les birds
		ab = can.create_image(px0, py0, image=image_ab2, anchor=NW)
...
def tire_gland():
	...
	image_mac = ImageTk.PhotoImage(file='img/gland.png')							#les dechets
	mac = can.create_image(pxg0, pyg0, image=image_mac, anchor=NW)	
...
def tire_gland2():
	...
	image_mac2 = ImageTk.PhotoImage(file='img/gland.png')							#les dechets
...
def tirer(x,y):
	global px,py,tps,flag1,image_fum,fum,v1,v0,angr,angd,vx0,vy0,combo,ab,poss
...
def stop1():
	global px0,py0,angr,angd,v0,v1,vx0,vy0,flag1,ab,ly,lx				# Stop bird normal
	timer_deplace.stop()
	timer_trace.stop()
	lx = px
	ly = py
...
def stop2():															# Stop birds quand sort limite		
...
def boot():
	...
	print bt,
	if bt == 0:
		print "lancement lvl, gland et temps"
		timer_gland.start()
		timer_niveau.start()
		timer_temps.start()
	if bt == 2:
		print  "supr Logo + stop boot"
		timer_boot.stop()
		can.delete(logo)
	bt = bt + 1
...
image_ab = ImageTk.PhotoImage(file='img/squirrel.png')
image_ab2 = ImageTk.PhotoImage(file='img/mac.png')							#les birds
ab = can.create_image(px0, py0, image=image_ab, anchor=NW)


image_champ = ImageTk.PhotoImage(file='img/cmp.png')							
champ = can.create_image(570, -30, image=image_champ, anchor=NW)
image_champ2 = ImageTk.PhotoImage(file='img/cmp2.png')							
champ2 = can.create_image(-60, -30, image=image_champ2, anchor=NW)
